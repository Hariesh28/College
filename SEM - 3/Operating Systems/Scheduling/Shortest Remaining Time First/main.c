/*
    A PROGRAM TO IMPLEMENT SHORTEST REMAINING TIME SCHEDULING ALGORITHM (PREEMPTIVE)
...
// Dequeue based on shortest burst time
process* dequeue_shortest_job (queue *queue){
    ...
    int shortest_process_index = queue->front;

    // Check every process from the start until the rear (exclusive)
    while (current_process_index != queue->rear){

        if (queue->processes[current_process_index]->remaining_time < return_process->remaining_time){

            return_process = queue->processes[current_process_index];
            shortest_process_index = current_process_index;
        }
    ...
    if (shortest_process_index == queue->front) queue->front = (queue->front + 1) % queue->capacity;

    // Shift all the process after the dequeued process one step before
    else {

        int current_index = shortest_process_index;
        int next_index;
...
int main (int argc, char *argv[]){
    ...
    while (true){
        ...
        if (!current_process)
            current_process = dequeue_shortest_job(&ready_queue);
