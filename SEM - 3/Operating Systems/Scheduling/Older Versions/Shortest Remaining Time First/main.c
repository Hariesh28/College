    ...
    while(true){
        ...
        for(int i = 0; i < no_of_programs; i++){
            ...
            if(programs[i].arrival_time == clock){
                ...
                // If the new process has less burst time than the current executing process
                if(current_program and new_exe->remaining_time < current_program->remaining_time){

                    // Add current process back to ready queue
                    enqueue(&ready_queue, current_program);

                    // Assign current process to the new process
                    current_program = new_exe;
                }

                // Else add the new process to the ready queue
                else
